...
def create_user_links(user_id: UUID, request: Request) -> List[Link]:
    ...
    return [
        create_link(
            rel, str(request.url_for(action, user_id=str(user_id))), method, action_desc
        )
...
def generate_pagination_links(
    ...
    links = [
        ...
        create_pagination_link(
            "last",
            base_url,
            {"skip": max(0, (total_pages - 1) * limit), "limit": limit},
        ),
    ]
    ...
    if skip + limit < total_items:
        links.append(
            create_pagination_link(
                "next", base_url, {"skip": skip + limit, "limit": limit}
            )
        )
    ...
    if skip > 0:
        links.append(
            create_pagination_link(
                "prev", base_url, {"skip": max(skip - limit, 0), "limit": limit}
            )
        )
