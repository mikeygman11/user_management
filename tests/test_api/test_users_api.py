"""Module to test users and the API endpoints"""
...
async def test_login_locked_user(async_client, locked_user):
    ...
    assert response.status_code == 400
    assert "Account locked due to too many failed login attempts." in response.json().get(
        "detail", ""
    )
