...
from app.utils.link_generation import (create_link,
                                       create_user_links,
...
def test_create_user_links(mock_request):
    ...
    assert (
        normalize_url(str(links[1].href)) == f"http://testserver/update_user/{user_id}"
    )
    assert (
        normalize_url(str(links[2].href)) == f"http://testserver/delete_user/{user_id}"
    )
